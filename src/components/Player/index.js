...
class Player extends React.Component {
    ...
    updateProgress(ev) {
        ...
        let val = 0
        if (ev.target.currentTime > 0) {
            val = ((100 / ev.target.duration) * ev.target.currentTime).toFixed(2)
        }
        statusbar.style.width = val + '%'
        if (ev.target.duration <= ev.target.currentTime) {
    ...
    renderNav() {
        const { currentUser, activeTrackId, isPlaying, entities, playlist, isInShuffleMode, onSetToggle, onActivateIteratedTrack, onLike, onTogglePlayTrack, onSetShuffleMode, volume } = this.props
                ...
                <div className='player-content'>
                    ...
                    <div className='player-content-action'>
                        <ButtonInline onClick={() => onSetToggle(toggleTypes.PLAYLIST)}>
                            <i className='fa fa-th-list' />{playlist.lenght}
                        </ButtonInline>
                    </div>
                    <div className='player-content-action'>
